...
for date in all_date:
    ...
    for i in range(1,len(cdata)):
        ...
        # 策略判斷
        if index == 0:
            if c_time.strftime('%H%M') >= '0910':
                if big_b_amount > big_s_amount:
                    index = 1
                    order_time = c_time
                    order_price = c_price
                    continue
                elif big_b_amount < big_s_amount:
                    index = -1
                    order_time = c_time
                    order_price = c_price
                    continue
        elif index == 1:
            if c_time.strftime('%H%M') >= '1325':
                index = 999
                cover_time = c_time
                cover_price = c_price
            ...
            if c_time.strftime('%H%M') >= '1325':
                ...
                print('空進場:%s 價位:%s 出場:%s 價位:%s' %(order_time,order_price,cover_time,cover_price))
                break
    ...
    if index == 999:
        ...
        # ax1.plot_date(order_time,order_price,'yo')
        ...
        # plt.show()
        pass
        #plt.savefig(('%s strategy %s.png') % (path, date))
        #plt.clf()
        # break
